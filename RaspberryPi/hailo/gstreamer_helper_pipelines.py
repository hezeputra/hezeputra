...
def get_source_type(input_source):
    ...
    if input_source.startswith("/dev/video"):
        ...
        return 'ximage'
    elif input_source.startswith('url'):
        return 'url'
    else:
        return 'file'
...
def SOURCE_PIPELINE(video_source, video_width=640, video_height=640, video_format='RGB', name='source', no_webcam_compression=False):
    ...
    http_url = "rtsp://admin:wika%40123@192.168.2.64:554/Streaming/Channels/101/"
    ...
    if source_type == 'usb':
        ...
        source_element = (
            f'uridecodebin uri="{http_url}" name={name}_uridecodebin ! '
            f'{QUEUE(name=f"{name}_queue_convert")} ! '
            f'videoconvert name={name}_videoconvert ! '
        )
    else:
        source_element = (
...
def DISPLAY_PIPELINE(video_sink='autovideosink', sync='true', show_fps='false', name='hailo_display'):
    ...
    display_pipeline = (
        ...
        f'{QUEUE(name=f"{name}_q")} ! '
        f'fakesink '
        # f'fpsdisplaysink name={name} video-sink={vi
        # f'fakesink 'deo_sink} sync={sync} text-overlay={show_fps} signal-fps-measurements=true '
    )
